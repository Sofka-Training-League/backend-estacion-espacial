...
const consultarOCrearUsuarioFromAuth1 = async (req, callback) => {
  ...
  await baseDeDatos
    .collection("usuarios")
    .findOne({ email: userA0.email }, async (err, response) => {
...
const consultarOCrearUsuarioFromAuth2 = async (datos, callback) => {
  ...
  await baseDeDatos
    .collection("usuarios")
    .findOne({ email: datos.email }, async (err, response) => {
      console.log("response consulta bd usuario", response);
      if (response) {
...
const consultarVendedoresActivos = async (callback) => {
  ...
  await baseDeDatos
    .collection("usuarios")
    .find({ rol: "Vendedor", estado: "Autorizado" })
    .toArray(callback);
};
...
const consultarVendedoresBloqueados = async (callback) => {
  ...
  await baseDeDatos
    .collection("usuarios")
    .find({ rol: "Vendedor", estado: "No autorizado" }, callback);
};
